...
def compute_checklist_likelihood(pres_abs_logit, obs_logit, m, rel_cells, n_cells):
    ...
    # multiple observations per grid cell, the "rel_cells" array should match
    # them up. Specifically, entry rel_cells[i] should contain the index j so
    ...
    summed_liks = jnp.bincount(rel_cells, weights=rel_log_probs, length=n_cells)
    ...
    obs_per_cell = jnp.bincount(rel_cells, weights=not_miss, length=n_cells)
