...
class MultiOutputGP(PresenceAbsenceModel):
    ...
    @staticmethod
    def get_standard_model_cv_over_w_prior_variance(
            X, y, variances_to_try, cv_save_dir, n_folds=4, n_kernels=10,
        ...
        def get_model(w_prior):
            ...
            default_kernel = partial(
                MultiOutputGP.build_default_kernel, n_dims,
                n_kernels=n_kernels, n_outputs=n_outputs, add_bias=add_bias,
                kern_var_trainable=kern_var_trainable, rbf_var=rbf_var,
                bias_var=bias_var)

            cur_kernel = default_kernel(w_prior=w_prior)

            model_fun = partial(MultiOutputGP, n_inducing=n_inducing,
                                n_latent=n_kernels, kernel=cur_kernel,
                                maxiter=maxiter)
        ...
        for cur_variance in variances_to_try:
            ...
            cur_mean_score = MultiOutputGP.cross_val_score(
                X, y, model_fun, save_dir=cv_save_dir, n_folds=n_folds)
        ...
        best_variance = variances_to_try[best_score]
